...
echo -e "${BLUE}🦉 prometheus Build Script${NC}"
echo "================================"
...
uv run pyinstaller prometheus.spec --noconfirm

RELEASE_DIR="dist/release"
mkdir -p "$RELEASE_DIR"

BINARY_NAME="prometheus-${VERSION}-${OS_NAME}-${ARCH_NAME}"

if [ "$OS_NAME" = "windows" ]; then
    if [ ! -f "dist/prometheus.exe" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    BINARY_NAME="${BINARY_NAME}.exe"
    cp "dist/prometheus.exe" "$RELEASE_DIR/$BINARY_NAME"
    ...
    if [ ! -f "dist/prometheus" ]; then
        echo -e "${RED}Build failed: Binary not found${NC}"
        exit 1
    fi
    cp "dist/prometheus" "$RELEASE_DIR/$BINARY_NAME"
